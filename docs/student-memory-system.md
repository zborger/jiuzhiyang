...
不要每次提问/点赞都立即写 KV。采用以下策略：

```
前端 localStorage 实时记录所有操作
        ↓
触发同步到 KV 的时机（3 选 1，哪个先到算哪个）：
  1. 用户关闭/离开页面时（beforeunload 事件）
  2. 每 5 分钟自动同步一次（setInterval）
  3. 累积满 10 条新消息时
        ↓
一次 API 调用批量写入 KV
```
...
**效果估算：**
- 70 个学生每天实际 KV 写入：约 100-200 次（而非几千次）
- Cloudflare KV 免费层每天 1000 次写入，绰绰有余
- 读取无限制，admin 查看数据随便看

**数据一致性保障：**
- localStorage 是主存储（即时可用）
- KV 是备份和汇总存储（定期同步）
- 即使同步失败，下次打开页面时重试
